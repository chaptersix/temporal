...
func (b *SpecBuilder) NewCompiledSpec(spec *schedulepb.ScheduleSpec) (*CompiledSpec, error) {
	if spec == nil {
		return nil, errors.New("schedule spec is nil")
	}
...
func canonicalizeSpecWithFaults(spec *schedulepb.ScheduleSpec, faults analysisFaults) (*schedulepb.ScheduleSpec, error) {
	...
	if spec.StartTime != nil {
		if err := spec.StartTime.CheckValid(); err != nil {
			return nil, fmt.Errorf("invalid start time: %w", err)
		}
	}
	if spec.EndTime != nil {
		if err := spec.EndTime.CheckValid(); err != nil {
			return nil, fmt.Errorf("invalid end time: %w", err)
		}
	}
	if spec.Jitter != nil {
		if err := spec.Jitter.CheckValid(); err != nil {
			return nil, fmt.Errorf("invalid jitter: %w", err)
		}
		if spec.Jitter.AsDuration() < 0 {
			return nil, errors.New("jitter is negative")
		}
	}
...
func validateStructuredCalendar(scs *schedulepb.StructuredCalendarSpec) error {
	if scs == nil {
		return errors.New("structured calendar is nil")
	}
	var errs []string
...
func validateInterval(i *schedulepb.IntervalSpec) error {
	...
	if i.Interval != nil {
		if err := i.Interval.CheckValid(); err != nil {
			return fmt.Errorf("invalid interval duration: %w", err)
		}
	}
	if i.Phase != nil {
		if err := i.Phase.CheckValid(); err != nil {
			return fmt.Errorf("invalid phase duration: %w", err)
		}
	}
...
func (cs *CompiledSpec) getNextTime(
	...
	for {
		if !nominal.IsZero() {
			...
			budget.at(nominal)
			if err := budget.tick(workExcludedRetry); err != nil {
				return GetNextTimeResult{}, err
			}
		}

		var err error
		nominal, err = cs.rawNextTime(after, budget)
		if err != nil {
			return GetNextTimeResult{}, err
		}
		after = nominal
	...
	following, err := cs.rawNextTime(nominal, budget)
	if err != nil {
		return GetNextTimeResult{}, err
	}
...
func (cs *CompiledSpec) rawNextTime(
	after time.Time,
	budget *iterationBudget,
) (time.Time, error) {
	...
	for _, cal := range cs.calendar {
		budget.at(after)
		if err := budget.tick(workInclusionSource); err != nil {
			return time.Time{}, err
		}
		next, err := cal.next(after, budget)
		if err != nil {
			return time.Time{}, err
		}
		if !next.IsZero() {
	...
	for _, iv := range cs.spec.Interval {
		...
		if err := budget.tick(workInterval); err != nil {
			return time.Time{}, err
		}
	...
	return time.Unix(minTimestamp, 0).UTC(), nil
}
...
func (cs *CompiledSpec) excluded(
	nominal time.Time,
	budget *iterationBudget,
) (bool, error) {
	for _, excal := range cs.excludes {
		budget.at(nominal)
		if err := budget.tick(workExclusion); err != nil {
			return false, err
		}
