...
func newSchedulerConformance(t *rapid.T, paused bool) *schedulerConformance {
	...
	runner := &schedulerConformance{
		...
		config: model.Config{
			StartTime:         schedulerPropertyStartTime,
			Interval:          defaultInterval,
			CatchupWindow:     defaultCatchupWindow,
			RecentActionLimit: scheduler.RecentActionCount,
			MaxGenerated:      8,
	...
	runner.quiesce(t, nil, 0)
	return runner
}

func (r *schedulerConformance) apply(t *rapid.T, event model.Event) {
	t.Helper()
	beforeCalls := len(r.env.services.Start.Calls())
...
func (r *schedulerConformance) quiesce(t *rapid.T, effects []model.Effect, beforeCalls int) {
	t.Helper()
	_, err := r.env.engine.DrainTasks(t.Context(), r.env.ref, schedulerConformanceDrainLimit)
	require.NoError(t, err)

	starts := startEffects(effects)
	calls := r.env.services.Start.Calls()[beforeCalls:]
	require.Len(t, calls, len(starts), "model and CHASM emitted different start counts")
	for i, start := range starts {
		r.requestByAction[start.Action.ID] = calls[i].Request.GetRequestId()
		outcome, transitionErr := model.Transition(r.config, r.state, model.StartSucceeded{
			ActionID: start.Action.ID,
			RunID:    calls[i].Response.GetRunId(),
		})
...
func (r *schedulerConformance) requireObservation(t *rapid.T) {
	...
	require.Equal(t, want.ConflictToken, decodeConflictToken(description.GetConflictToken()))
...
func startEffects(effects []model.Effect) []model.StartWorkflow {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		t.Repeat(map[string]func(*rapid.T){
			...
			"describe": func(t *rapid.T) {
				runner.apply(t, model.Describe{})
			},
			"": func(t *rapid.T) {
				runner.requireObservation(t)
			},
		})
	})
}
