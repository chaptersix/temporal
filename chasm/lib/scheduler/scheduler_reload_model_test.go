...
func TestSchedulerReloadModel(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		if len(env.runnableTasks(t)) == 0 {
			t.Fatal("expected an active pure generator task")
		}
		env.reload(t)
		env.drain(t)
		if len(env.workflows.snapshot().starts) != 1 {
			t.Fatal("reloaded generator did not start workflow")
		}
		...
		if len(sideEffects) == 0 {
			t.Fatal("expected an active side-effect task")
		}
		env.reload(t)
		env.drain(t)
		if len(env.workflows.snapshot().starts) != 2 {
			t.Fatal("reloaded invoker did not start workflow")
		}
		...
		if !env.internal(t).closed {
			t.Fatal("reload reopened a closed execution")
		}
