...
// Attaches completion callbacks to running workflows that were migrated from
// V1 (workflow-backed) schedulers. This ensures the CHASM scheduler receives
// completion notifications for workflows that were started before migration.
type MigrationCallbackTask struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MigrationCallbackTask) Reset() {
	*x = MigrationCallbackTask{}
	mi := &file_temporal_server_chasm_lib_scheduler_proto_v1_tasks_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MigrationCallbackTask) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MigrationCallbackTask) ProtoMessage() {}

func (x *MigrationCallbackTask) ProtoReflect() protoreflect.Message {
	mi := &file_temporal_server_chasm_lib_scheduler_proto_v1_tasks_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use MigrationCallbackTask.ProtoReflect.Descriptor instead.
func (*MigrationCallbackTask) Descriptor() ([]byte, []int) {
...
const file_temporal_server_chasm_lib_scheduler_proto_v1_tasks_proto_rawDesc = "" +
	...
	"\x0eBackfillerTask\"\x17\n" +
	"\x15MigrationCallbackTaskBGZEgo.temporal.io/server/chasm/lib/scheduler/gen/schedulerpb;schedulerpbb\x06proto3"
...
var file_temporal_server_chasm_lib_scheduler_proto_v1_tasks_proto_goTypes = []any{
	...
	(*MigrationCallbackTask)(nil),    // 5: temporal.server.chasm.lib.scheduler.proto.v1.MigrationCallbackTask
