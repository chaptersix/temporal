...
// ReloadExecution reconstructs an execution from a persistence snapshot while
// retaining its execution identity and physical task queues.
...
// RunnableTasks returns due CHASM tasks in deterministic delivery order.
...
// ExecuteTask delivers one physical CHASM task. Successful and stale tasks are
// acknowledged; handler failures retain the queued task for retry.
...
// DrainTasks executes runnable tasks until none remain or limit is reached.
func (e *Engine) DrainTasks(
	...
	if limit < 0 {
		return 0, fmt.Errorf("chasmtest: task drain limit must be non-negative: %d", limit)
	}
...
func (e *Engine) executeSideEffectTask(
	...
	return TaskExecutionResult{LogicalTasksExecuted: 1}, nil
}
